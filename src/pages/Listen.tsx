...
function RotaryDial({ onDial }: { onDial: (digit: number) => void }) {
  ...
  function handleSelect(digit: number) {
    ...
    // no pull — target stays 0, which Framer Motion won't animate, so
...
export default function Listen() {
  ...
  function handleDial(digit: number) {
    setDialedDigits((prev) => (prev.length >= CODE_LENGTH ? [digit] : [...prev, digit]))
  }

  return (
    <>
      <main className="relative flex min-h-screen flex-col items-center justify-center gap-8 bg-white px-8 py-12 text-black lg:w-1/2 lg:px-12">
        <BackButton />
